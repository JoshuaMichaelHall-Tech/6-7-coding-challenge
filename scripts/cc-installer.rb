...
class CodingChallengeInstaller
  ...
  def install_scripts
    ...
    if Dir.exist?(lib_source)
      ...
      # Update the cc_config.rb file to support our customizations
      update_config_module(lib_target)
    end
  end

  # Update the config module with new structure
  def update_config_module(lib_target)
    config_module_path = File.join(lib_target, 'cc_config.rb')
    
    if File.exist?(config_module_path)
      config_content = File.read(config_module_path)
      
      # Update calculation methods to account for variable phase lengths
      updated_content = config_content.gsub(/def current_phase.*?end/m) do |match|
        <<-RUBY
  def current_phase
    config = load
    day = current_day
    ...
    phases.keys.map(&:to_i).max || 1
      ...
      # Update week_in_phase calculation
      updated_content = updated_content.gsub(/def week_in_phase.*?end/m) do |match|
        <<-RUBY
  def week_in_phase
    config = load
    day = current_day
    ...
    ((day_in_phase - 1) / days_per_week) + 1
      ...
      # Update phase_dir to use dynamic phase information
      updated_content = updated_content.gsub(/def phase_dir.*?end/m) do |match|
        <<-RUBY
  def phase_dir
    config = load
    phase = current_phase
    config["challenge"]["phases"][phase.to_s]["dir"]
      ...
      # Add/update log_dir method
      updated_content = updated_content.gsub(/def log_dir.*?end/m) do |match|
        <<-RUBY
  def log_dir
    config = load
    config["paths"]["log_dir"]
      ...
      # If log_dir method doesn't exist, add it
      unless updated_content.include?("def log_dir")
        updated_content = updated_content.gsub(/def bin_dir.*?end/m) do |match|
          match + "\n\n" + <<-RUBY
  def log_dir
    config = load
    config["paths"]["log_dir"]
      ...
      # Update the DEFAULT_CONFIG with new structure
      updated_content = updated_content.gsub(/DEFAULT_CONFIG = \{.*?\}.freeze/m) do |match|
        <<-RUBY
  DEFAULT_CONFIG = {
    "version" => "#{VERSION}",
    "user" => {
    ...
    "challenge" => {
      ...
      File.write(config_module_path, updated_content)
      puts "Updated configuration module" if @options[:verbose]
    end
  end
