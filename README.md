...
This project uses a centralized installer-only approach for script maintenance:

- All scripts are generated by the `cc-installer.rb` script
- To modify any script functionality, edit the appropriate `generate_*` method in the installer
- Run `ccupdate` or `ruby ~/bin/cc-installer.rb --update` after making changes

This approach simplifies maintenance while preserving all functionality.
